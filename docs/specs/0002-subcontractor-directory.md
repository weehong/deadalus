...
- The handoff's Playwright host limitation proved stale during implementation.
  Chromium, Firefox and WebKit launch successfully here without installing
  system packages. Actual runs and their scope are recorded in the
  [verification document](0002-subcontractor-directory-verification.md).
- The name key column and its unique index are created in the same migration
  as the tables, so there is no backfill and no database collation
  requirement; the service derives the key on every write. Local migration and
  constraint checks passed; the hosted Supabase database was not contacted.
- Local fallback database URLs still name `public`. Set `DATABASE_URL`
  explicitly with `schema=daedalus2`; the preexisting inconsistency is tracked
