...
- End-to-end tests intercept two seams at the browser's edge — the provider's
  ...
  generated key pair. Feature specs add their own API intercept beside these
  (the Subcontractor Directory added `/api/v1/subcontractors*`), scoped to the
  tests that reach that screen rather than applied to every auth test.
