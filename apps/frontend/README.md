...
1. The Console opens on `/projects` after Sign in. Projects lists developments with search, paging and Structure counts; `/projects/new` creates one, `/projects/$id` manages its Blocks, Storeys and Units with atomic batch creation, and `/projects/$id/unit-types` manages its Unit Type catalogue. Subcontractors opens the searchable, paged Directory; `/subcontractors/new` creates a Subcontractor with its first Member, and `/subcontractors/$id` supports rename, Member management and confirmed deletion. These routes share the guarded sidebar and content layout. See the [Directory spec](../../docs/specs/0002-subcontractor-directory.md) and [verification record](../../docs/specs/0002-subcontractor-directory-verification.md). The reference demo under `src/features/example` is also guarded and uses the same shell, but has no navigation entry: visit `/example` directly after signing in. It wires together TanStack Query + Table, React Hook Form + Zod, a Nivo chart, a Zustand store, i18n, and each library's devtools against the real matches API and database. To remove it, delete `src/features/example`, `src/routes/_console/example.ts`, `src/store/useExampleStore.ts`, `e2e/example.spec.ts`, and the `example` translation keys; regenerate the route tree by starting Vite. Keep the Console shell and its Projects and Subcontractors routes. See the [root README](../../README.md#the-example-slice-is-disposable) for the backend cleanup.
